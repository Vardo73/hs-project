...
export default class HubspotService{
  ...
  //Recibe info de CSAT y actualiza en Hubspot
  public async csat_sentiment(id_ticket:string,csat_sentiment:string,info_browser:string,comment?:string): Promise<boolean>{
    ...
    const properties: any = {
      ...
      info_csat_browser:info_browser
    };

    // Agregar respuestas_csat si se proporciona comment
    if (comment) {
      properties.respuestas_csat = comment;
    }
  ...
  //Metodo para obtener fecha con formato para Hubspot
  ...
  //Función de consulta de contacto en hubspot
  ...
  //Actualización de solo comentario en ticket de hubspot
  ...
  //Función para consultar info de IP del usuario
  public async getGeoInfo(ip: string) {
    try {
      const TOKEN_IPINFO = '5053b7fb30e81c'; // Reemplaza con tu token de API de IPinfo
      const response = await fetch(`https://ipinfo.io/${ip}?token=${TOKEN_IPINFO}`);
